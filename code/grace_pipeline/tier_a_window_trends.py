...
Artefacts (datasets/output/tier_a_windows/)
  ...
  jasechko_per_recipe_trends__<start>_<end>.csv   per-window trend table,
...
EXISTING_FULL = (_CODE_REPO / "datasets" / "output" / "tier_a_full"
                 / "jasechko_per_recipe_trends.csv")
EXISTING_GRACE = (_CODE_REPO / "datasets" / "output" / "tier_a_grace_only"
                  / "jasechko_per_recipe_trends.csv")
...
def do_windows(centres, basins_file: Path = None) -> int:
    ...
    for _, row in distinct.iterrows():
        ...
        out = OUT_DIR / f"jasechko_per_recipe_trends__{s}_{e}.csv"
