...
Truth NetCDFs, synthetic Level-2 files, recovered cubes and the per-planet
synthetic per-recipe trend tables (`datasets/output/tier_a_synth_<planet>/`,
which the calibration gate, the scoring and the recipe-skill, rolling-window
and boundary scripts read) are regenerated from the specifications (about
14.5 hours per planet); the scripts that read only the banked scores and
summary tables (`09`, `14`, `17`, `19`) run from this repository as shipped. Two inputs of the
