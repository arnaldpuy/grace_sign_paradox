...
real_dt <- if (file.exists(real_csv)) fread(real_csv) else
  fread(cmd = paste("gunzip -c", shQuote(paste0(real_csv, ".gz"))))

# The two-tier table (276 aquifers) is restricted to the 73-aquifer cohort and
# re-indexed to the cohort's aquifer_idx, so both sides of every gate share the
# same support (D062; the first report compared the synthetic 73 against all
# 276 real aquifers).
cohort_cells <- fread(file.path(here::here(),
                            "datasets/jasechko_2024/cell_masks_0p5deg.csv"))[, .(n_cells = .N), by = .(aquifer_idx, Study_area)]
real_dt <- real_dt[Study_area %in% cohort_cells$Study_area]
real_dt[, aquifer_idx := cohort_cells$aquifer_idx[match(Study_area,
                                                          cohort_cells$Study_area)]]
stopifnot(uniqueN(real_dt$aquifer_idx) == 73L)
...
stopifnot(setequal(unique(synth_dt$aquifer_idx), unique(real_dt$aquifer_idx)))
